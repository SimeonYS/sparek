...
class SparekPipeline:

    # Database setup
    conn = sqlite3.connect('sparek.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `sparek`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM sparek WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO sparek (date, title, link, content)"
